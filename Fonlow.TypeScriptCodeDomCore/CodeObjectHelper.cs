...
namespace Fonlow.TypeScriptCodeDom
{
    ...
    public class CodeObjectHelper
    {
        ...
        /// <param name="w"></param>
        /// <param name="o"></param>
        static void GenerateCodeFromAttributeDeclarationForClass(CodeAttributeDeclaration e, TextWriter w, CodeGeneratorOptions o)
        {
...
https://angular.io/guide/dependency-injection-in-action
        ...
        protected static void GenerateCodeFromAttributeDeclarationCollectionForClass(CodeAttributeDeclarationCollection e, TextWriter w, CodeGeneratorOptions o)
        {
            if (e.Count == 0)
                return;
        ...
        static bool WriteCodeCommentStatement(CodeCommentStatement commentStatement, TextWriter w, CodeGeneratorOptions o)
        {
            ...
            if (commentStatement.Comment.DocComment)
            {
                ...
                {
                    w.WriteLine();
                    var lines = commentStatement.Comment.Text.Split(lineEnds, StringSplitOptions.RemoveEmptyEntries);
        ...
        static void WriteCodeParameterDeclarationExpressionCollection(CodeParameterDeclarationExpressionCollection parameterDeclarations, TextWriter w, CodeGeneratorOptions o)
        {
            var pairs = parameterDeclarations.OfType<CodeParameterDeclarationExpression>()
                .Select(d =>
                {
                    ...
                    var alreadyNullable = typeText.EndsWith("| null", StringComparison.Ordinal);
        ...
        protected static bool WriteCodeCommentStatementCollection(CodeCommentStatementCollection comments, TextWriter w, CodeGeneratorOptions o)
        {
            if (comments.Count == 0)
                return false;
        ...
        static bool WriteCodeRegionDirective(CodeRegionDirective codeRegionDirective, TextWriter w)
        {
        ...
        static readonly string[] lineEnds = new string[] { "\r\n", "\n", "\r" };
        ...
        /// <param name="typeDeclaration"></param>
        /// <returns></returns>
        protected static string GetTypeOfTypeText(CodeTypeDeclaration typeDeclaration)
        {
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        protected static string GetTypeOfTypeText(TypeOfType t) // v3.1.2
        {
            return typeOfTypeTexts[(int)t];
        }

        protected static TypeOfType GetTypeOfType(CodeTypeDeclaration typeDeclaration)
        {
            return typeDeclaration.IsEnum
                ? TypeOfType.IsEnum
                : typeDeclaration.IsInterface
        ...
        protected static string GetTypeParametersExpression(CodeTypeDeclaration typeDeclaration)
        {
        ...
        protected static string GetBaseTypeExpression(CodeTypeDeclaration typeDeclaration)
        {
            var baseTypes = typeDeclaration.BaseTypes
                ...
                .ToList();
            if (baseTypes.Count != 0 && !typeDeclaration.IsEnum)
            {
        ...
        protected static string GetCodeTypeReferenceText(CodeTypeReference codeTypeReference)
        {
